...
import { type CloudinaryAsset, formatBytes, thumbUrl } from "@/lib/cloudinary";

type Props = {
  asset: CloudinaryAsset;
  selected: boolean;
  onSelect: (id: string, additive: boolean) => void;
  onOpen: (asset: CloudinaryAsset) => void;
  onStar: (id: string) => void;
  view: "grid" | "list";
};

export default function MediaCard({ asset, selected, onSelect, onOpen, onStar, view }: Props) {
  const isVideo = asset.resource_type === "video";
  const name = asset.display_name || asset.original_filename;
  ...
  if (view === "list") {
    return (
      <motion.div
        ...
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        onClick={(e) => onSelect(asset.public_id, e.metaKey || e.ctrlKey || e.shiftKey)}
      ...
      >
        <div className="w-12 h-12 rounded-md overflow-hidden bg-muted shrink-0 relative">
          <img src={thumbUrl(asset, 120)} loading="lazy" className="w-full h-full object-cover" alt="" />
          {isVideo && (
        ...
        <div className="hidden md:block w-24 text-xs text-muted-foreground">{formatBytes(asset.bytes)}</div>
        <div className="hidden md:block w-28 text-xs text-muted-foreground">{date}</div>
        <button
          onClick={(e) => {
            e.stopPropagation();
            onStar(asset.public_id);
          }}
  ...
  return (
    ...
    >
      <div className="aspect-square bg-muted relative overflow-hidden">
        ...
        >
          <Star className={`w-3.5 h-3.5 ${asset.starred ? "fill-gold text-gold" : ""}`} />
        </button>
        {selected && (
